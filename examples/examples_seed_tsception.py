"""TSCeption with the SEED Dataset
======================================
In this case, we introduce how to use TorchEEG to train a TSCeption on the SEED dataset for emotion classification.
"""
...
#
# We use the SEED dataset supported by TorchEEG. Here, we set an EEG sample to 1 second long and include 200 data points. In offline preprocessing, all EEG signals are debaselined and normalized, and the preprocessed EEG signals are stored in the local IO. In online processing, all EEG signals are converted into Tensors for input into neural networks.
#

dataset = SEEDDataset(io_path=f'./tmp_out/examples_seed_tsception/seed',
...
k_fold = KFoldGroupbyTrial(n_splits=5, split_path='./tmp_out/examples_seed_tsception/split')
...
for i, (train_dataset, val_dataset) in enumerate(k_fold.split(dataset)):
    ...
    model = TSCeption(num_electrodes=len(SEED_CHANNEL_LOCATION_DICT),
                      num_classes=3,
                      num_T=15,
                      num_S=15,
                      in_channels=1,
