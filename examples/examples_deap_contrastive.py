...
contras_dataset = DEAPDataset(
    io_path=f'./tmp_out/examples_deap_ccnn/deap',
    root_path='./tmp_in/data_preprocessed_python',
    offline_transform=transforms.Compose([
    ...
    online_transform=transforms.Compose([
        transforms.ToTensor(),
        transforms.Contrastive(transforms.Compose(
...
test_accuracy_list = []
for i, ((train_dataset, val_dataset), (contras_train_dataset,
                                      contras_val_dataset)) in enumerate(
                                          zip(k_fold.split(dataset),
                                              k_fold.split(contras_dataset))):
    # Initialize the model
    model = Extractor(in_channels=4,
                      grid_size=(9, 9),
                      num_classes=2,
                      dropout=0.5)
    ...
    train_loader = DataLoader(contras_train_dataset,
                              batch_size=128,
                              shuffle=True,
                              num_workers=4)

    val_loader = DataLoader(contras_val_dataset,
                            batch_size=128,
                            shuffle=True,
                            num_workers=4)
    ...
    # model.load_state_dict(trainer.extractor.state_dict())
    ...
    # Do 50 rounds of training
    trainer.fit(train_loader, val_loader, max_epochs=50)
    test_results = trainer.test(val_loader)
    test_accuracy_list.append(test_results[0]['test_accuracy'])

print(f'Average test accuracy: {np.mean(test_accuracy_list)}')
