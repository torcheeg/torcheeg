...
from torcheeg.model_selection.k_fold_trial_per_subject import KFoldTrialPerSubject
...
file_handler = logging.FileHandler('./tmp_out/examples_tsception.log')
...
if __name__ == "__main__":
    ...
    k_fold = KFoldTrialPerSubject(n_splits=10, split_path=f'./tmp_out/split', shuffle=True)
    ...
    for i, (train_dataset, test_dataset) in enumerate(k_fold.split(dataset)):
        ...
        for t in range(epochs):
            ...
            if val_acc > best_val_acc:
                ...
                torch.save(model.state_dict(), f'./tmp_out/model{i}.pt')

        # Test
        test_loader = DataLoader(test_dataset, batch_size=batch_size, shuffle=False)

        model.load_state_dict(torch.load(f'./tmp_out/model{i}.pt'))
        test_acc, test_loss = valid(test_loader, model, loss_fn)

        logger.info(f"Test Error {i}: {(100*test_acc):>0.1f}%, Avg loss: {test_loss:>8f} \n")

        test_accs.append(test_acc)
        test_losses.append(test_loss)

    # Print results
    logger.info(f"Test Error: \n Accuracy: {100*np.mean(test_accs):>0.1f}%", f"Avg loss: {np.mean(test_losses):>8f}")