...
class StrokePatientsMIDataset(BaseDataset):
    ...
    .. code-block:: python
        ...
        dataset = StrokePatientsMIDataset(root_path='your unzipped root path',
                                ...
                                overlap = 0,
                                from_edf = False,  # load the data in sourcedata dir
    ...
    Args:
        ...
        from_edf (bool): Whether load data from edffile directory (the processed version provided from authors, in which data have undergone baseline removal and 8-40 bandpass operation). If set to False, it will load the raw data from the sourcedata dir. (default: :obj:`True`)
    ...
    @classmethod
    def process_record(cls, from_edf, **kwargs):
        if from_edf:
            gen = cls.process_record_edf(**kwargs)
        else:
            gen = cls.process_record_mat(**kwargs)
        while 1:
            try:
                yield next(gen)
    ...
    def set_records(self, root_path, from_edf, **kwargs):
        if from_edf:
