Numpy-based Transforms
=================================

transforms.CWTSpectrum
----------------------------------------

.. autoclass:: torcheeg.transforms.CWTSpectrum

transforms.BandDifferentialEntropy
----------------------------------------

.. autoclass:: torcheeg.transforms.BandDifferentialEntropy

transforms.BandPowerSpectralDensity
------------------------------------------

.. autoclass:: torcheeg.transforms.BandPowerSpectralDensity

transforms.BandMeanAbsoluteDeviation
--------------------------------------

.. autoclass:: torcheeg.transforms.BandMeanAbsoluteDeviation

transforms.BandKurtosis
--------------------------------------

.. autoclass:: torcheeg.transforms.BandKurtosis

transforms.BandSkewness
--------------------------------------

.. autoclass:: torcheeg.transforms.BandSkewness

transforms.MeanStdNormalize
--------------------------------------

.. autoclass:: torcheeg.transforms.MeanStdNormalize

transforms.MinMaxNormalize
--------------------------------------

.. autoclass:: torcheeg.transforms.MinMaxNormalize

transforms.PickElectrode
--------------------------------------

.. autoclass:: torcheeg.transforms.PickElectrode

transforms.To2d
--------------------------------------

.. autoclass:: torcheeg.transforms.To2d

transforms.ToGrid
--------------------------------------

.. autoclass:: torcheeg.transforms.ToGrid

transforms.ToInterpolatedGrid
--------------------------------------

.. autoclass:: torcheeg.transforms.ToInterpolatedGrid

transforms.Concatenate
--------------------------------------

.. autoclass:: torcheeg.transforms.Concatenate
    